...
import { IIntermediatePartService } from './parts/intermediate/intermediatePart.js';
...
export abstract class Layout extends Disposable implements IWorkbenchLayoutService {
	...
	private intermediatePartView!: ISerializableView;
	...
	protected initLayout(accessor: ServicesAccessor): void {
		...
		accessor.get(IIntermediatePartService);
	...
	protected createWorkbenchLayout(): void {
		...
		const intermediatePart = this.getPart(Parts.INTERMEDIATE_PART);
		...
		this.intermediatePartView = intermediatePart;
		...
		const viewMap = {
			...
			[Parts.INTERMEDIATE_PART]: this.intermediatePartView
		};
		...
		for (const part of [titleBar, editorPart, activityBar, panelPart, sideBar, statusBar, auxiliaryBarPart, bannerPart, intermediatePart]) {
	...
	private arrangeEditorNodes(nodes: { editor: ISerializedNode; sideBar?: ISerializedNode; auxiliaryBar?: ISerializedNode; intermediate?: ISerializedNode }, availableHeight: number, availableWidth: number): ISerializedNode {
		if (!nodes.sideBar && !nodes.auxiliaryBar && !nodes.intermediate) {
		...
		// Add intermediate part after sidebar (if sidebar is on left) or before editor (if sidebar is on right)
		if (nodes.intermediate) {
			if (this.stateModel.getRuntimeValue(LayoutStateKeys.SIDEBAR_POSITON) === Position.LEFT) {
				// Insert after sidebar but before editor
				result.splice(-1, 0, nodes.intermediate);
			} else {
				// Insert before editor when sidebar is on right
				result.splice(0, 0, nodes.intermediate);
			}
			nodes.editor.size -= nodes.intermediate.size;
		}
	...
	private arrangeMiddleSectionNodes(nodes: { editor: ISerializedNode; panel: ISerializedNode; activityBar: ISerializedNode; sideBar: ISerializedNode; auxiliaryBar: ISerializedNode; intermediate: ISerializedNode }, availableWidth: number, availableHeight: number): ISerializedNode[] {
		...
		const intermediateSize = nodes.intermediate.size;
		...
		if (!isHorizontal(panelPostion)) {
			...
			nodes.editor.size = availableWidth - activityBarSize - sideBarSize - panelSize - auxiliaryBarSize - intermediateSize;
			...
			if (sideBarPosition === Position.LEFT) {
				result.push(nodes.auxiliaryBar);
				result.splice(0, 0, nodes.intermediate); // Add intermediate part after sidebar
				result.splice(0, 0, nodes.sideBar);
				result.splice(0, 0, nodes.activityBar);
			} else {
				result.splice(0, 0, nodes.auxiliaryBar);
				result.push(nodes.intermediate); // Add intermediate part before sidebar when sidebar is on right
				result.push(nodes.sideBar);
			...
			const editorSectionWidth = availableWidth - activityBarSize - (sideBarNextToEditor ? 0 : sideBarSize) - (auxiliaryBarNextToEditor ? 0 : auxiliaryBarSize) - intermediateSize;
			...
			const editorNodes = this.arrangeEditorNodes({
				...
				intermediate: nodes.intermediate
	...
	private createGridDescriptor(): ISerializedGrid {
		...
		const intermediateNode: ISerializedLeafNode = {
			type: 'leaf',
			data: { type: Parts.INTERMEDIATE_PART },
			size: 200, // Default width for intermediate part
